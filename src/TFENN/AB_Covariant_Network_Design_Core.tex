...
$R_i\mapsto B_i^{(r)}$ 先消去分子 $i$ 的右侧 gauge，训练前冻结的 typed covariant basis $C$ 再负责线性 lift、非线性 lift、A/B 混合、B/B 混合与直接 A 型力输出。运行时只学习 channel mixing 与不变量标量系数；全项目绝不使用群平均。
\end{designbox}
...
\begin{enumerate}
  ...
  \item 将网络结构定为：同边 typed 数据 $\to$ 不变量 gates 与 $C$ 输出 $\to$ 有向消息 $\to$ 邻居求和 $\to$ typed residual；共享程序一次性输出所有分子的力。
...
\begin{equation}
  \mathcal T=\{A,B^{(r)}:r\in\mathcal R_B\},
  \qquad
  \rho_\tau:G\to \mathrm{GL}(V_\tau),\quad \tau\in\mathcal T,
\end{equation}
...
主构架只保留四个阶段；所有实现细节均下沉到第\,\ref{sec:c-math}--\ref{sec:subflows} 节。
...
\{h_{i,0}^{\tau}\}_{\tau\in\mathcal T}
\xrightarrow{L\times(\,s\to\gamma,\ C\to m,\ \sum_j,\ \text{typed update}\,)}
\{h_{i,L}^{\tau}\}
...
\label{eq:main-flow}
\end{equation}
一次 root preprocessing 产生整片结构数据；同一套 edge/node 程序对所有 $i$ 共享，并同时给出所有受力。
...
对输出类型 $\tau\in\mathcal T\cup\{\one\}$，广义 $C$ 的完整空间定义为
\begin{equation}
  \boxed{
  C_{\tau\leftarrow\Sigma,\alpha}
...
\begin{tikzpicture}[node distance=8mm]
  ...
  \node[flowStrong,text width=4.7cm,right=of r] (h) {$h_{i,0}^{A}=\alpha(X_i)$\\$h_{i,0}^{B^{(r)}}=B_i^{(r)}$};
...
\subsection{一层 typed edge/message update}
...
\begin{tikzpicture}[node distance=6mm and 14mm]
  ...
  \node[flow,text width=5.0cm,below=of msg] (upd) {$\sum_j\to M_{i,\ell}^{\tau}$\\typed residual $\to h_{i,\ell+1}^{\tau}$};
  \draw[arr] (z)--(inv);
  \draw[arr] (z)--(cov);
  \draw[arr] (inv)--(msg);
  \draw[arr] (cov)--(msg);
...
\begin{enumerate}
  ...
  \item 同边 A/B 数据必须先通过 typed $C$ 耦合，再做邻居聚合；否则会丢失“哪个位置对应哪个姿态”。
  ...
  \item 全项目绝对禁止群平均。输入生成元只用于离线方程 $CS_q=T_qC$，不进入网络运行时。
  ...
  \item 共享 edge program 与对邻居求和保证固定根下的分子重标号等变；根的选择是一次性结构约定。
  \item 世界力只由 $F_i=O_0f_i$ 解码。直接 A-head 保证协变，但“直接力”本身不等价于自动满足保守力条件。
\end{enumerate}

\begin{designbox}
\textbf{最终设计判据。}
未来任何实现都应能逐项证明：一次 root frame、B 消右 gauge、所有 hidden tensors 共用 root action、typed $C$ 完整基、不变量门控、对称聚合、直接 A-head，以及 $F_i=O_0f_i$ 的世界坐标解码。
\end{designbox}
