...
from core.models import TFENN_1, TFENN_2, TFENN_3, TFENN_3_1, TFENN_3_2, TFENN_3_3, TFENN_3_4
...
R_HIDDEN       = 16          # channel: R-branch hidden channels
NUM_X_LAYERS   = 2         # hidden_layer_num for x-branch
NUM_R_LAYERS   = 1           # hidden_layer_num for R-branch
R2X_CHANNELS   = 16         # r2xlayerchannel
NUM_XXGCONV_LAYERS=2
XXGCONV_CHANNELS=None
XX_CHANNELS = 16
OUT_CHANNELS   = 2           # we predict F(3) + M(3)
...
class ShiftedLeakyReLU(nn.Module):
    ...
    y = LeakyReLU(x - shift)
    """
    def __init__(self, negative_slope: float = 0.01, shift: float = 1.0, inplace: bool = False):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return F.leaky_relu(x - self.shift, negative_slope=self.negative_slope, inplace=self.inplace)


MODELNAME     =f"TFENN1_X{X_HIDDEN}{NUM_X_LAYERS}{XX_CHANNELS}_R{R_HIDDEN}{NUM_R_LAYERS}_RX{R2X_CHANNELS}_XXGC{XXGCONV_CHANNELS}{NUM_XXGCONV_LAYERS}"
model  =  TFENN_3_4(x_hidden_channels=X_HIDDEN,num_x_layers=NUM_X_LAYERS,R_hidden_channels=R_HIDDEN,num_R_layers=NUM_R_LAYERS,
...
experiment.log_parameters({
    ...
    "hidden_layer_num_x": NUM_X_LAYERS,
    "hidden_layer_num_R": NUM_R_LAYERS,
    "channel_x": X_HIDDEN,
    "channel_R": R_HIDDEN,
    "R2xlayerchannel": R2X_CHANNELS,
})
